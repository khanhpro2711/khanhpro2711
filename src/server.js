...
async function handleZaloWebhook(req, res) {
  ...
  } else if (lowerText.startsWith('/stk')) {
    await handlePayoutAccountCommand(event, user);
...
async function handleShopeeLink(event, user, group, shopeeUrl) {
  ...
  const cashbackRate = store.cashbackShareRate(group?.id ?? null);
  const platformFeeRate = Math.max(1 - cashbackRate, 0);
...
💰 Cashback: ${formatPercent(cashbackRate)} hoa hồng được duyệt (đã trừ ${formatPercent(platformFeeRate)} phí nền tảng).
...
async function handlePayoutAccountCommand(event, user) {
  ...
  if (!['bank', 'momo'].includes(method) || !accountInfo) {
    await sendZaloMessage(event.replyTarget, 'Cú pháp thêm STK: /stk momo 09xxxxxxxx hoặc /stk bank VCB 0123456789 NGUYEN VAN A');
    return;
  }
  const account = store.addPayoutAccount({ userId: user.id, method, accountInfo });
  await sendZaloMessage(event.replyTarget, `✅ Đã lưu tài khoản nhận tiền mặc định: ${account.method.toUpperCase()} - ${account.accountInfo}
💡 Sau này có thể rút nhanh bằng: /rut 50000`);
}

async function handleWithdrawCommand(event, user) {
  const [, amountText, maybeMethod, ...accountParts] = event.text.trim().split(/\s+/);
  const amount = Number(amountText);
  const savedAccount = store.defaultPayoutAccount(user.id);
  const method = ['bank', 'momo'].includes(maybeMethod) ? maybeMethod : savedAccount?.method;
  const accountInfo = ['bank', 'momo'].includes(maybeMethod) ? accountParts.join(' ') : savedAccount?.accountInfo;
  const account = store.userSummary(user.id);
  if (!amount || !method || !accountInfo) {
    await sendZaloMessage(event.replyTarget, 'Cú pháp rút tiền: /rut 50000 hoặc /rut 50000 momo 09xxxxxxxx. Thêm STK mặc định bằng: /stk bank VCB 0123456789 NGUYEN VAN A');
    return;
  }
  ...
  await sendZaloMessage(event.replyTarget, `✅ Đã tạo yêu cầu rút ${formatVnd(amount)} qua ${method.toUpperCase()} - ${accountInfo}. Admin sẽ xử lý thủ công trong MVP.`);
}
